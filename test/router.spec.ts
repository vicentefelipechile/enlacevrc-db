...
describe('Router and Authentication', () => {
  ...
  it('should return 405 for disallowed methods', async () => {
    ...
    expect(await response.json()).toEqual({ success: false, error: 'Method HEAD not allowed' });
  });
  ...
  it('should return 400 for POST requests to /discord-settings without server ID', async () => {
    ...
    await waitOnExecutionContext(ctx);
    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ success: false, error: 'A server ID is required for POST requests (e.g., /discord-settings/some_id)' });
  });
  ...
  it('should return 400 for GET requests to /discord-settings without server ID', async () => {
    ...
    await waitOnExecutionContext(ctx);
    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ success: false, error: 'A server ID is required for GET requests (e.g., /discord-settings/some_id)' });
  });
  ...
  it('should return 400 for PUT requests to /discord-settings without server ID', async () => {
    ...
    await waitOnExecutionContext(ctx);
    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ success: false, error: 'A server ID is required for PUT requests (e.g., /discord-settings/some_id)' });
  });
  ...
  it('should return 400 for DELETE requests to /discord-settings without server ID', async () => {
    ...
    await waitOnExecutionContext(ctx);
    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ success: false, error: 'A server ID is required for DELETE requests (e.g., /discord-settings/some_id)' });
  });
  ...
  it('should return 405 for disallowed methods on /discord-settings', async () => {
    ...
    await waitOnExecutionContext(ctx);
    expect(response.status).toBe(405);
    expect(await response.json()).toEqual({ success: false, error: 'Method PATCH not allowed.' });
  });
});
